//
//  InAppPurchaseProvider+SecurityBreach.m
//  app
//
...
#define KNOWN_TRANSACTIONS_KEY              @"AppleSecurityBreachIAPKeys"

//You can get this from itunes connect under [App]->In-App Purchases->View or generate a shared secret
#define ITC_CONTENT_PROVIDER_SHARED_SECRET  @"98a852db383b466485cb3836eebb3d02"

@interface InAppPurchaseProvider (SecurityBreachInternal)

+ (NSDictionary*) dictionaryFromPlistData: (NSData*) data;
+ (NSDictionary*) dictionaryFromJSONData: (NSData*) data;
+ (BOOL) isReceiptValid: (SKPaymentTransaction*) transaction;
+ (BOOL) isTransactionUnique: (NSString*) transactionId;
...
static char *kVerfifcationCompletedAssociationKey = "NSURLConnectionVerificationCompleted";

@implementation InAppPurchaseProvider (SecurityBreach)

static NSMutableDictionary* transactionsReceiptStorageDictionary = nil;

+ (BOOL) verifyTransaction: (SKPaymentTransaction*) transaction {
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        transactionsReceiptStorageDictionary = [NSMutableDictionary new];
    });
    
    BOOL isOK = [self isReceiptValid: transaction];
    if( !isOK ) 
        return isOK;
...
+ (void) verifyReceipt: (NSData*) transactionReceipt completion: (kInAppPurchaseVerificationComplete) completion {
...
#if DEBUG
    NSString* serverURL = @"https://sandbox.itunes.apple.com/verifyReceipt";
#else
    NSString* serverURL = @"https://buy.itunes.apple.com/verifyReceipt";
#endif
    ...
    NSURLConnection* conn = [[NSURLConnection alloc] initWithRequest: request delegate: (id<NSURLConnectionDelegate>)self];
    objc_setAssociatedObject(conn, 
                             &kVerfifcationCompletedAssociationKey,  
                             completion, 
...
+ (NSDictionary*) dictionaryFromJSONData: (NSData*) data {
    ...
    NSError* error = nil;
    NSDictionary* dictionary = [parser objectFromString: string error: &error];
...
+ (BOOL) isReceiptValid:(SKPaymentTransaction *)transaction {
    if( !(transaction && [transaction.transactionReceipt length] > 0) ) {
        return NO;
    }
    ...
    if(![self isTransactionUnique: transactionId] ) 
        return NO;
    
    if( !checkReceiptSecurity(transactionPurchaseInfo, signature, (__bridge CFDateRef)purchaseDate) )
        return NO; 
...
+ (NSString*) decodeBase64: (NSString*) encoded {
    char* outputString = NewBase64Decode([encoded UTF8String], [encoded length], NULL);
    NSString* retVal = [[NSString alloc] initWithCString: outputString encoding: NSUTF8StringEncoding];
    free(outputString);
    return retVal;
}
...
+ (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    ...
    //Were we validated?
    kInAppPurchaseVerificationComplete completion = objc_getAssociatedObject(connection, &kVerfifcationCompletedAssociationKey);
        
    if( completion ) {
        completion([self doesTransactionInfoMatchReceipt: responseString]);
    }
}
